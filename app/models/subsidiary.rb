class Subsidiary < ApplicationRecord
  ...
  validate :cnpj_can_have_13_type_of_characters
  validate :cnpj_must_be_valid

  private

  def cnpj_can_have_13_type_of_characters
    return if cnpj.blank?
    return if cnpj.length != 18
    if cnpj.count("0-9") != 14 || cnpj.count(".") != 2 || cnpj.count("/") != 1 || cnpj.count("-") != 1
      errors.add(:cnpj, 'os caracteres aceitos para CNPJ são: números(0 a 9), ponto(.), barra(/) e traço(-)')
    end
  end
